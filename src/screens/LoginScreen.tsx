...
import { Mail, Lock, Eye, EyeOff, AlertCircle, ArrowRight, X } from 'lucide-react';
...
export default function LoginScreen() {
  ...
  const handleGoogleClick = () => {
    if (isGoogleConfigured) {
      ...
      if (gis?.initTokenClient) {
        const tokenClient = gis.initTokenClient({
          ...
          callback: async (response: any) => {
            if (!response.access_token) { setError('Google sign-in was cancelled.'); return; }
            setLoading(true);
            setError('');
            try {
              ...
              if (result.success) {
                const saved = localStorage.getItem('smartlecture_user');
                let role = 'student';
                if (saved) try { role = JSON.parse(saved).role || 'student'; } catch {}
                navigate(`/${role}`, { replace: true });
      ...
      setError('Google Identity Services not loaded. Try again.');
      return;
    }
  ...
  const handleGoogleSubmit = async () => {
    if (!googleName.trim() || !googleEmail.trim()) { setError('Please fill in your name and email.'); return; }
    setLoading(true);
    setError('');
    try {
      const result = await googleLogin(`demo-${btoa(googleEmail)}`);
      if (result.success) {
        const saved = localStorage.getItem('smartlecture_user');
        let role = 'student';
        if (saved) try { role = JSON.parse(saved).role || 'student'; } catch {}
    ...
    finally { setLoading(false); setGoogleModal(false); }
  };
  ...
  return (
    <div className={cn('min-h-screen flex', darkMode ? 'dark' : '')}>
      ...
      {googleModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setGoogleModal(false)}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 w-full max-w-sm mx-4 shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">Connect with Google</h3>
              <button type="button" onClick={() => setGoogleModal(false)} className="p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Enter your Google account details to sign in (demo mode — no real OAuth configured).</p>
            <div className="space-y-3">
              <div>
                ...
                <input type="text" value={googleName} onChange={e => setGoogleName(e.target.value)} placeholder="Jane Doe"
                  className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-[var(--must-gold)]/30 focus:border-[var(--must-gold)]/50 outline-none text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">Google Email</label>
                <input type="email" value={googleEmail} onChange={e => setGoogleEmail(e.target.value)} placeholder="jane@gmail.com"
                  className="w-full px-3 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-[var(--must-gold)]/30 focus:border-[var(--must-gold)]/50 outline-none text-sm" />
              </div>
              <button type="button" onClick={handleGoogleSubmit} disabled={loading}
                className="w-full py-2.5 bg-[#4285F4] hover:bg-[#3367D6] text-white rounded-xl font-semibold text-sm transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
                {loading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <><svg className="w-4 h-4" viewBox="0 0 24 24"><path fill="#fff" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"/><path fill="#fff" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/><path fill="#fff" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/><path fill="#fff" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/></svg> Continue with Google</>}
              </button>
            </div>
          </div>
        </div>
