...
export default function LoginScreen() {
  ...
  const handleGoogleClick = () => {
    ...
    setError('To use Google sign-in, add VITE_GOOGLE_CLIENT_ID to .env');
  };
  ...
  const handleGitHubClick = async () => {
    ...
    setError('');
    try {
      const result = await googleLogin('github-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');
      if (result.success) {
        const saved = localStorage.getItem('smartlecture_user');
        let role = 'student';
        if (saved) try { role = JSON.parse(saved).role || 'student'; } catch {}
  ...
  return (
    <div className={cn('min-h-screen flex', darkMode ? 'dark' : '')}>
        ...
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleLogin} className="premium-card p-6 space-y-5">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={handleGoogleClick} disabled={loading || gitHubLoading} className="flex-1 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex items-center justify-center gap-2 text-sm disabled:opacity-60">
              ...
              <button type="button" onClick={handleGitHubClick} disabled={loading || gitHubLoading} className="flex-1 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors flex items-center justify-center gap-2 text-sm disabled:opacity-60">
                {gitHubLoading ? (
      ...
      {/* Google will work when VITE_GOOGLE_CLIENT_ID is set in .env */}
    </div>
  );
}
